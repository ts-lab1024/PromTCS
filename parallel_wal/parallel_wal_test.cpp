...
namespace tsdb::parallel_wal {
    ...
    // ========================================================================
    // Roundtrip tests for Gorilla-compressed WAL samples
    // ========================================================================

    // Helper: encode N samples, decode, and verify data matches
    void verifyGorillaRoundtrip(tsdb::tsdbutil::TreeRefSample* samples, uint32_t count) {
        // Encode
        std::string encoded = leveldb::log::treeSamplesGorilla(samples, count);
        ASSERT_FALSE(encoded.empty());
        ASSERT_EQ(static_cast<uint8_t>(encoded[0]),
                  static_cast<uint8_t>(leveldb::log::kSampleGorilla));

        // Decode
        leveldb::Slice record(encoded);
        std::vector<tsdb::tsdbutil::TreeRefSample> decoded;
        bool ok = leveldb::log::treeSamplesGorilla(record, &decoded);
        ASSERT_TRUE(ok);
        ASSERT_EQ(decoded.size(), count);

        // Verify each sample
        for (uint32_t i = 0; i < count; i++) {
            ASSERT_EQ(samples[i].mid, decoded[i].mid)
                << "mid mismatch at index " << i;
            ASSERT_EQ(samples[i].sgid, decoded[i].sgid)
                << "sgid mismatch at index " << i;
            ASSERT_EQ(samples[i].t, decoded[i].t)
                << "t mismatch at index " << i;
            ASSERT_DOUBLE_EQ(samples[i].v, decoded[i].v)
                << "v mismatch at index " << i;
            ASSERT_EQ(samples[i].txn, decoded[i].txn)
                << "txn mismatch at index " << i;
        }
    }

    TEST(GorillaRoundtripTest, SingleSample) {
        tsdb::tsdbutil::TreeRefSample s(100, 1, 1000000, 42.5, 1);
        verifyGorillaRoundtrip(&s, 1);
    }

    TEST(GorillaRoundtripTest, TypicalBatch) {
        // Simulate typical WAL batch: 16 samples from same series,
        // fixed 15s interval timestamps, varying values, sequential txn.
        constexpr uint32_t N = 16;
        tsdb::tsdbutil::TreeRefSample samples[N];
        for (uint32_t i = 0; i < N; i++) {
            samples[i].sgid = 42;
            samples[i].mid  = 3;
            samples[i].t    = 1600000000 + i * 15;  // fixed 15s interval
            samples[i].v    = 0.5 + i * 0.03;        // slowly increasing
            samples[i].txn  = i + 1;
        }
        verifyGorillaRoundtrip(samples, N);
    }

    TEST(GorillaRoundtripTest, SameDoubleValues) {
        // All samples have the same double value → XOR = 0 for all.
        ...
        for (uint32_t i = 0; i < N; i++) {
            ...
            samples[i].v    = 3.14159;  // same for all
            samples[i].txn  = i + 100;
        }
        verifyGorillaRoundtrip(samples, N);
    }

    TEST(GorillaRoundtripTest, IrregularTimestamps) {
        // Timestamps with varying intervals → DoD non-zero cases.
        constexpr uint32_t N = 6;
        tsdb::tsdbutil::TreeRefSample samples[N];
        int64_t timestamps[N] = {1000, 1015, 1015, 1045, 1046, 1100};
        for (uint32_t i = 0; i < N; i++) {
            samples[i].sgid = 7;
            samples[i].mid  = 2;
            samples[i].t    = timestamps[i];
            samples[i].v    = static_cast<double>(i);
            samples[i].txn  = i;
        }
        verifyGorillaRoundtrip(samples, N);
    }

    TEST(GorillaRoundtripTest, TxnJump) {
        // txn values with jumps → delta changes, tests txn encoding.
        constexpr uint32_t N = 5;
        tsdb::tsdbutil::TreeRefSample samples[N];
        int64_t txns[N] = {10, 11, 20, 21, 22};
        for (uint32_t i = 0; i < N; i++) {
            samples[i].sgid = 5;
            samples[i].mid  = 9;
            samples[i].t    = 2000 + i;
            samples[i].v    = 1.0;
            samples[i].txn  = txns[i];
        }
        verifyGorillaRoundtrip(samples, N);
    }

    TEST(GorillaRoundtripTest, NegativeAndSpecialFloats) {
        // Test negative values, zero, very small, and very large doubles.
        constexpr uint32_t N = 5;
        tsdb::tsdbutil::TreeRefSample samples[N];
        double values[N] = {-1.5, 0.0, 1e-10, 1e10, 0.0};
        for (uint32_t i = 0; i < N; i++) {
            samples[i].sgid = 1;
            samples[i].mid  = 1;
            samples[i].t    = 1000 + i;
            samples[i].v    = values[i];
            samples[i].txn  = i;
        }
        verifyGorillaRoundtrip(samples, N);
    }

    TEST(GorillaRoundtripTest, LargeBatch) {
        // Stress test with 256 samples (typical ITEM_NUM).
        constexpr uint32_t N = 256;
        tsdb::tsdbutil::TreeRefSample samples[N];
        for (uint32_t i = 0; i < N; i++) {
            samples[i].sgid = 100;
            samples[i].mid  = 50;
            samples[i].t    = 1000000 + i * 30;
            samples[i].v    = std::sin(static_cast<double>(i) * 0.1);
            samples[i].txn  = 5000 + i;
        }
        verifyGorillaRoundtrip(samples, N);
    }
