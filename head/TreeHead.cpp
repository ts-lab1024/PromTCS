...
void* TreeHead::label_arena_alloc(size_t size, size_t alignment) {
    // Ensure at least one chunk exists.
    if (label_arena_.empty()) {
        label_arena_.emplace_back(kLabelArenaChunkSize);
        label_arena_chunk_offset_ = 0;
    }

    // Align offset within current chunk.
    size_t aligned = (label_arena_chunk_offset_ + alignment - 1) & ~(alignment - 1);

    // If request doesn't fit in the current chunk, start a new chunk.
    if (aligned + size > kLabelArenaChunkSize) {
        // Large allocation gets its own dedicated chunk.
        size_t chunk_sz = (size > kLabelArenaChunkSize) ? size : kLabelArenaChunkSize;
        label_arena_.emplace_back(chunk_sz);
        label_arena_chunk_offset_ = 0;
        aligned = 0;
    }

    void* p = &label_arena_.back()[aligned];
    label_arena_chunk_offset_ = aligned + size;
    return p;
}
...
TreeHead::TreeHead(const std::string& dir, const std::string& snapshot_dir,
  ...
  memset(source_array_, 0, sizeof(source_array_));

  source_labels_ptr_.resize(max_source_num_, nullptr);
  metric_labels_ptr_.resize(max_metric_num_, nullptr);
  ...
  forward_index_.resize(max_metric_num_);
  for (size_t i = 0; i < max_metric_num_; i++) {
...
TreeHead::TreeHead(size_t max_source_num, size_t max_metric_num,
                   const std::string& dir, const std::string& log_path, const std::string& snapshot_dir,
                   leveldb::DB* db, slab::TreeSeries* tree_series, bool sync)
    : max_source_num_(max_source_num),
      max_metric_num_(max_metric_num),
      pwal_m_(this, wal_num, log_path),
      log_path_(log_path),
      last_source_group_id_(0),
      last_metric_id_(0),
      hash_shards_(STRIPE_SIZE),
      inverted_index_(log_path_),
      dir_(dir),
      db_(db),
...
TreeHead::~TreeHead() {
  ...
  // 清理 MemSource 对象，避免内存泄漏
  for (uint64_t i = 0; i <= last_source_group_id_ && i < MAX_SOURCE_NUM; i++) {
    if (source_array_[i] != nullptr) {
      delete source_array_[i];
      source_array_[i] = nullptr;
    }
  }

  // 手动调用共享标签池对象的析构函数（内存来自 arena，不需 delete）。
  for (auto* p : source_labels_ptr_) { if (p) p->~vector(); }
  source_labels_ptr_.clear();
  for (auto* p : metric_labels_ptr_) { if (p) p->~Label(); }
  metric_labels_ptr_.clear();
    ...
    for (const auto & existing_log : existing_logs) {
        ...
        while (r.ReadRecord(&record, &scratch)) {
            if (record.data()[0] == leveldb::log::kSample ||
                record.data()[0] == leveldb::log::kSampleGorilla) {
                rs.clear();
                if (record.data()[0] == leveldb::log::kSample) {
                    leveldb::log::treeSamples(record, &rs);
                } else {
                    leveldb::log::treeSamplesGorilla(record, &rs);
                }
...
const label::Label& TreeHead::get_metric_label(uint16_t mid) const {
    static const label::Label empty;
    if (mid == 0 || mid >= metric_labels_ptr_.size() || !metric_labels_ptr_[mid])
        return empty;
    return *metric_labels_ptr_[mid];
}

const label::Labels& TreeHead::get_source_labels(uint64_t sgid) const {
    static const label::Labels empty;
    if (sgid == 0 || sgid >= source_labels_ptr_.size() || !source_labels_ptr_[sgid])
        return empty;
    return *source_labels_ptr_[sgid];
}

void TreeHead::reconstruct_labels(uint64_t sgid, uint16_t mid, label::Labels& out) const {
    if (mid < metric_labels_ptr_.size() && metric_labels_ptr_[mid])
        out.push_back(*metric_labels_ptr_[mid]);
    if (sgid < source_labels_ptr_.size() && source_labels_ptr_[sgid]) {
        const auto& src = *source_labels_ptr_[sgid];
...
//    TreeMemSeries* tms = (TreeMemSeries*)(thmap_get(forward_index_[metric_id],&source_id, sizeof(uint64_t)));
    ...
    // Reconstruct full label set from shared pools directly into lset.
    reconstruct_labels(source_id, metric_id, lset);
...
//                std::cout<<"mid: "<<i<<" sgid: "<<j<<" lset: "<<"NULL"<<std::endl;
                ...
                label::Labels tmp;
                reconstruct_labels(j, i, tmp);
                std::cout<<"mid: "<<i<<" sgid: "<<j<<" max_time: "<<tms->max_time_
                         <<" lset: "<<label::lbs_string(tmp)<<std::endl;
            }
        }
    }
}
...
uint64_t TreeHead::get_or_create_source_id(const label::Labels& lset, bool& is_exist,uint64_t epoch) {
  ...
  if (source_group == NULL) {
    ...
    if(source_group){
      ...
      return (uint64_t)(uintptr_t)source_group;
    }
    is_exist = false;
    uint64_t sgid = last_source_group_id_.fetch_add(1) + 1;
    thmap_put(source_thmap_, source_group_str.c_str(), source_group_str.size(), (void*)(uintptr_t)sgid);
    // Populate shared source label pool (lset[0] is __name__, rest are source labels).
    void* _am_s = label_arena_alloc(sizeof(label::Labels), alignof(label::Labels));
    source_labels_ptr_[sgid] = new (_am_s) label::Labels(lset.begin() + 1, lset.end());
    source_locks_[hash1%source_lock_num_].unlock();
    return sgid;
  }
  return (uint64_t)(uintptr_t)source_group;
...
uint16_t TreeHead::get_or_create_metric_id(const label::Labels& lset, bool& is_exist, uint64_t epoch) {
  ...
  if (metric == NULL) {
    ...
    if(metric){
      ...
      return (uint16_t)(uintptr_t)metric;
    }
    is_exist = false;
    uint16_t mid = last_metric_id_.fetch_add(1) + 1;
    thmap_put(metric_thmap_, metric_str.c_str(), metric_str.size(), (void*)(uintptr_t)mid);
    // Populate shared metric label pool (lset[0] is __name__).
    void* _am_m = label_arena_alloc(sizeof(label::Label), alignof(label::Label));
    metric_labels_ptr_[mid] = new (_am_m) label::Label(lset[0]);
    metric_locks_[hash1%metric_lock_num_].unlock();
    return mid;
  }
  return (uint16_t)(uintptr_t)metric;
...
MemSource *TreeHead::get_mem_source(uint64_t sgid,const label::Labels & lset) {
    mutex_.read_lock();
    if(source_array_[sgid] != nullptr && source_array_[sgid]->is_exist){
        MemSource* result = source_array_[sgid];
        mutex_.unlock();
        return result;
    }
    mutex_.unlock();
...
//  }
  ...
  TreeMemSeries* ms = new TreeMemSeries(sgid,mid,slab_id,mem_source);
...
//    }
    slab_id = std::numeric_limits<uint32_t>::max();
    TreeMemSeries* ms = new TreeMemSeries(sgid, mid, slab_id);
    ms->flushed_txn_ = std::numeric_limits<int64_t>::max();
    set_to_forward_index(sgid, mid, ms);
    inverted_index_.add(sgid,lset);

    // Populate shared label pools if not already done (WAL recovery path).
    if (sgid < source_labels_ptr_.size() && !source_labels_ptr_[sgid]) {
        void* _am_s = label_arena_alloc(sizeof(label::Labels), alignof(label::Labels));
        source_labels_ptr_[sgid] = new (_am_s) label::Labels(lset.begin() + 1, lset.end());
    }
