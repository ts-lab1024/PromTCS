...
class TreeHead {
 private:
  ...
  // Configurable array dimensions (default to compile-time constants).
  size_t max_source_num_ = MAX_SOURCE_NUM;
  size_t max_metric_num_ = MAX_METRIC_NUM;

  // Arena for shared label allocations (chunked bump-pointer, chunks never relocate).
  // Objects allocated here are placement-new'd; destructors called manually at teardown.
  static constexpr size_t kLabelArenaChunkSize = 64 * 1024;  // 64 KB per chunk
  std::list<std::vector<uint8_t>> label_arena_;
  size_t label_arena_chunk_offset_ = 0;  // bump offset within current chunk
  void* label_arena_alloc(size_t size, size_t alignment = alignof(std::max_align_t));

  // Shared label pools: indexed by sgid (1-based) and mid (1-based).
  // Populated during get_or_create_*_id inside the per-hash spinlock.
  // Read-only after creation — no lock needed for access.
  std::vector<label::Labels*> source_labels_ptr_;
  std::vector<label::Label*>  metric_labels_ptr_;
 ...
 public:
  ...
  // Constructor with configurable array sizes (avoids wasting memory on small tests).
  TreeHead(size_t max_source_num, size_t max_metric_num,
  ...
  // Shared label pool accessors (lock-free after construction).
  const label::Label&  get_metric_label(uint16_t mid) const;
  const label::Labels& get_source_labels(uint64_t sgid) const;
  const label::Label*  get_metric_label_ptr(uint16_t mid) const { return metric_labels_ptr_[mid]; }
  const label::Labels* get_source_labels_ptr(uint64_t sgid) const { return source_labels_ptr_[sgid]; }

  // Reconstruct the full label set (metric label + source labels) from shared pools
  // into the caller-provided output vector (avoids intermediate allocation).
  void reconstruct_labels(uint64_t sgid, uint16_t mid, label::Labels& out) const;
...
//                  }
      ...
      std::thread bg_flush_db([this]() {
          sleep(1);
          MasstreeWrapper<slab::SlabInfo>::ti=threadinfo::make(threadinfo::TI_PROCESS, 1001);
          while (bg_flush_) {
              sleep(0.5);
