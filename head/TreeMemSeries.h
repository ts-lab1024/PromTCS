...
class TreeMemSeries {
 public:
  ...
  TreeMemSeries();
  TreeMemSeries(uint64_t sgid, uint16_t mid, uint32_t sid, uint64_t log_pos = 0);
  TreeMemSeries(uint64_t sgid, uint16_t mid, uint32_t sid, MemSource* mem_source, uint64_t log_pos = 0);
