...
TEST_F(HeadTest, GET_OR_CREATE_TEST) {
  ...
  for (int i = 0; i < metric_num; i++) {
    ...
    for (int j = 0; j < source_num; j++) {
      ...
      {
        label::Labels _lbs;
        hd->reconstruct_labels(ms->source_id_, ms->metric_id_, _lbs);
        std::cout<<"label set: "<<label::lbs_string(_lbs)<<"\tsgid: "<<ms->source_id_<<"\tmid: "<<ms->metric_id_<<std::endl;
      }
      lset.pop_back();
    }
    lset.clear();
...
TEST_F(HeadTest, FORWARD_INDEX_TEST) {
  ...
  for (int i = 0; i < metric_num; i++) {
    ...
    for (int j = 0; j < source_num; j++) {
      ...
      {
        label::Labels _lbs;
        hd->reconstruct_labels(nms->source_id_, nms->metric_id_, _lbs);
        std::cout<<"label set: "<<label::lbs_string(_lbs)<<"\tsgid: "<<nms->source_id_<<"\tmid: "<<nms->metric_id_<<std::endl;
      }
      lset.pop_back();
    }
    lset.clear();
