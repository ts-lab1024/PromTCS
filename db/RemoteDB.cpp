...
namespace tsdb {
    namespace db {
        ...
        RemoteDB::RemoteDB(const std::string& dir)
              : dir_(dir),
              pool_(32),
...
//                std::cout<<std::endl;
        ...
        leveldb::Status RemoteDB::setup(const std::string& dbpath) {
          ...
          head_ = new head::Head(dbpath,"",db_);
          db_->SetHead(head_);
          head_->enable_concurrency();
          head_->bg_clean_samples_logs();
          head_->set_no_log(true);

          leveldb::Options ms_opts;
          ms_opts.create_if_missing = true;
          ms_opts.use_log = false;
          head_->set_mergeset_manager(dbpath + "/mergeset", ms_opts);
