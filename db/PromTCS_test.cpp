...
//                    }
            ...
            void queryDevOps2(leveldb::DB* db, int64_t endtime,
                              leveldb::Cache* cache = nullptr, int iteration = 100) {
                int big_iteration = 100;
...
//                            std::cout<<std::endl;
                        ...
                        durations.push_back(t.since_start_nano());
                    }
                    std::sort(durations.begin(), durations.end());
                    int64_t duration = std::accumulate(durations.begin(), durations.end(), 0LL);
                    if (iteration > 10)
                        std::cout << "[1-1-1] duration(total):" << duration / 1000 << "us "<< "duration(avg):" << duration / 1000 / iteration << "us "<< "p50:" << durations[durations.size()/2] / 1000 << "us " << "p90:" << durations[durations.size()*9/10] / 1000 << "us " << "p99:" << durations[durations.size() * 99 / 100] / 1000 << "us "<< "samples:" << total_samples / iteration << ":"
...
//            {&matchers1[0], &matchers2[0]});
                    ...
                    int64_t duration = std::accumulate(durations.begin(), durations.end(), 0LL);
                    if (iteration > 10)
                        std::cout << "[1-1-12] duration(total):" << duration / 1000 << "us "<< "duration(avg):" << duration / 1000 / iteration << "us "<< "p50:" << durations[durations.size()/2] / 1000 << "us " << "p90:" << durations[durations.size()*9/10] / 1000 << "us " << "p99:" << durations[durations.size() * 99 / 100] / 1000 << "us "<< "samples:" << total_samples / iteration << std::endl;
                }
...
//            {&matchers1[0], &matchers2[0]});
                    ...
                    int64_t duration = std::accumulate(durations.begin(), durations.end(), 0LL);
                    if (iteration > 10)
                        std::cout << "[1-1-24] duration(total):" << duration / 1000 << "us "<< "duration(avg):" << duration / 1000 / iteration << "us "<< "p50:" << durations[durations.size()/2] / 1000 << "us " << "p90:" << durations[durations.size()*9/10] / 1000 << "us " << "p99:" << durations[durations.size() * 99 / 100] / 1000 << "us "<< "samples:" << total_samples / iteration << std::endl;
                }
...
//            {&matchers1[0], &matchers2[0]});
                    ...
                    int64_t duration = std::accumulate(durations.begin(), durations.end(), 0LL);
                    if (iteration > 10)
                        std::cout << "[1-1-all] duration(total):" << duration / 1000 << "us "<< "duration(avg):" << duration / 1000 / iteration << "us "<< "p50:" << durations[durations.size()/2] / 1000 << "us " << "p90:" << durations[durations.size()*9/10] / 1000 << "us " << "p99:" << durations[durations.size() * 99 / 100] / 1000 << "us "<< "samples:" << total_samples / iteration << std::endl;
                }
...
//              {&matchers1[host], &matchers2[0]});
                    ...
                    int64_t duration = std::accumulate(durations.begin(), durations.end(), 0LL);
                    if (iteration > 10)
                        std::cout << "[1-8-1] duration(total):" << duration / 1000 << "us "<< "duration(avg):" << duration / 1000 / iteration << "us "<< "p50:" << durations[durations.size()/2] / 1000 << "us " << "p90:" << durations[durations.size()*9/10] / 1000 << "us " << "p99:" << durations[durations.size() * 99 / 100] / 1000 << "us "<< "samples:" << total_samples / iteration << std::endl;
                }
...
//              {&matchers1[0], &matchers2[j]});
                    ...
                    int64_t duration = std::accumulate(durations.begin(), durations.end(), 0LL);
                    if (iteration > 10)
                        std::cout << "[5-1-1] duration(total):" << duration / 1000 << "us "<< "duration(avg):" << duration / 1000 / iteration << "us "<< "p50:" << durations[durations.size()/2] / 1000 << "us " << "p90:" << durations[durations.size()*9/10] / 1000 << "us " << "p99:" << durations[durations.size() * 99 / 100] / 1000 << "us "<< "samples:" << total_samples / iteration << std::endl;
                }
...
//              {&matchers1[0], &matchers2[j]});
                    ...
                    int64_t duration = std::accumulate(durations.begin(), durations.end(), 0LL);
                    if (iteration > 10)
                        std::cout << "[5-1-12] duration(total):" << duration / 1000 << "us "<< "duration(avg):" << duration / 1000 / iteration << "us "<< "p50:" << durations[durations.size()/2] / 1000 << "us " << "p90:" << durations[durations.size()*9/10] / 1000 << "us " << "p99:" << durations[durations.size() * 99 / 100] / 1000 << "us "<< "samples:" << total_samples / iteration << std::endl;
                }
...
//              {&matchers1[0], &matchers2[j]});
                    ...
                    int64_t duration = std::accumulate(durations.begin(), durations.end(), 0LL);
                    if (iteration > 10)
                        std::cout << "[5-1-24] duration(total):" << duration / 1000 << "us "<< "duration(avg):" << duration / 1000 / iteration << "us "<< "p50:" << durations[durations.size()/2] / 1000 << "us " << "p90:" << durations[durations.size()*9/10] / 1000 << "us " << "p99:" << durations[durations.size() * 99 / 100] / 1000 << "us "<< "samples:" << total_samples / iteration << std::endl;
                }
...
//              {&matchers1[0], &matchers2[j]});
                    ...
                    int64_t duration = std::accumulate(durations.begin(), durations.end(), 0LL);
                    if (iteration > 10)
                        std::cout << "[5-1-all] duration(total):" << duration / 1000 << "us "<< "duration(avg):" << duration / 1000 / iteration << "us "<< "p50:" << durations[durations.size()/2] / 1000 << "us " << "p90:" << durations[durations.size()*9/10] / 1000 << "us " << "p99:" << durations[durations.size() * 99 / 100] / 1000 << "us "<< "samples:" << total_samples / iteration << std::endl;
                }
...
//                {&matchers1[host], &matchers2[j]});
                    ...
                    int64_t duration = std::accumulate(durations.begin(), durations.end(), 0LL);
                    if (iteration > 10)
                        std::cout << "[5-8-1] duration(total):" << duration / 1000 << "us "<< "duration(avg):" << duration / 1000 / iteration << "us "<< "p50:" << durations[durations.size()/2] / 1000 << "us " << "p90:" << durations[durations.size()*9/10] / 1000 << "us " << "p99:" << durations[durations.size() * 99 / 100] / 1000 << "us "<< "samples:" << total_samples / iteration << std::endl;
                }
...
//            {&matchers1[0], &matchers2[0]});
                    ...
                    int64_t duration = std::accumulate(durations.begin(), durations.end(), 0LL);
                    if (iteration > 10)
                        std::cout << "[lastpoint] duration(total):" << duration / 1000 << "us "<< "duration(avg):" << duration / 1000 / iteration << "us "<< "p50:" << durations[durations.size()/2] / 1000 << "us " << "p90:" << durations[durations.size()*9/10] / 1000 << "us " << "p99:" << durations[durations.size() * 99 / 100] / 1000 << "us "<< "samples:" << total_samples / iteration << ":"
                                  << total_samples << std::endl;
                }
...
//                {&matchers1[host], &matchers3[j]});
                    ...
                    int64_t duration = std::accumulate(durations.begin(), durations.end(), 0LL);
                    if (iteration > 10)
                        std::cout << "[cpu-max-all-8] duration(total):" << duration / 1000 << "us "<< "duration(avg):" << duration / 1000 / big_iteration << "us "<< "p50:" << durations[durations.size()/2] / 1000 << "us " << "p90:" << durations[durations.size()*9/10] / 1000 << "us " << "p99:" << durations[durations.size() * 99 / 100] / 1000 << "us "<< "samples:" << total_samples / big_iteration << std::endl;
                }
...
//                {&matchers1[host], &matchers3[j]});
                    ...
                    int64_t duration = std::accumulate(durations.begin(), durations.end(), 0LL);
                    if (iteration > 10)
                        std::cout << "[double-groupby-5] duration(total):" << duration / 1000 << "us "<< "duration(avg):" << duration / 1000 / big_iteration << "us "<< "p50:" << durations[durations.size()/2] / 1000 << "us " << "p90:" << durations[durations.size()*9/10] / 1000 << "us " << "p99:" << durations[durations.size() * 99 / 100] / 1000 << "us "<< "samples:" << total_samples / big_iteration << std::endl;
                }
...
//                {&matchers1[host], &matchers3[j]});
                    ...
                    int64_t duration = std::accumulate(durations.begin(), durations.end(), 0LL);
                    if (iteration > 10)
                        std::cout << "[double-groupby-all] duration(total):" << duration / 1000 << "us "<< "duration(avg):" << duration / 1000 / big_iteration << "us "<< "p50:" << durations[durations.size()/2] / 1000 << "us " << "p90:" << durations[durations.size()*9/10] / 1000 << "us " << "p99:" << durations[durations.size() * 99 / 100] / 1000 << "us "<< "samples:" << total_samples / big_iteration << std::endl;
                }
            }
...
//                  {&matchers1->at(host), &matchers2->at(j)});
            ...
            leveldb::Status setup() {
                ...
                head_ = new head::TreeHead(1000005, 105, db_path, log_path,"",db_,tree_series_);
            ...
            leveldb::Status setup(const std::string& dbpath, const std::string& log_path) {
                ...
                if (fd >= 0) {
                    ftruncate(fd, static_cast<off_t>(8) * 1024 * 1024 * 1024);
                    close(fd);
                }
                slab::Setting *setting = new slab::Setting();
                ...
                head_ = new head::TreeHead(1000005, 105, dbpath, log_path,"",db_,tree_series_);
                db_->SetTreeHead(head_);
                return st;
            }
...
//            tsdbtest.db_->PrintLevel();
            ...
            // Pause compaction during query phase to prevent memory contention
            tsdbtest.db_->SetDisableCompaction(true);
            ...
            // Re-enable compaction after query phase
            tsdbtest.db_->SetDisableCompaction(false);
