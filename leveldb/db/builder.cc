  ...
  if (iter->Valid()) {
    ...
    for (; iter->Valid(); iter->Next()) {
      ...
      if (b == fnames.end()) {
        ...
        s = env->NewWritableFile(fname, &file);
        if (!s.ok()) {
          for (auto& p : builders) { p.second->Abandon(); delete p.second; }
          for (auto& p : files) { p.second->Close(); delete p.second; }
    ...
    for (auto& p : files) {
      if (s.ok()) {
        ...
        p.second->Close();  // try close anyway on error path
      }
      delete p.second;
    }
  }
